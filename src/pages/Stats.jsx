...
function Stats() {
    ...
    useEffect(() => {
        if (!statsData?.stats) {
            let unsubscribe = subscribeToStatsForUser((fetchedSoldAds, earnings) => {
                dispatch({
    ...
    if (!statsData?.stats) {
        ...
        return <NoDataMsg messageText={'No available stats'} />
    } else {
        return (
            <>
                        ...
                        statsData.stats.map(item => (
                            <Row key={item.ad.id}>
                                <Card className='flex-row'>
                                    <Card.Header style={{width: 250}}>
                                        <h3 className='text-success'>{item.ad.title}</h3>
                                    </Card.Header>
                                    <Card.Body className='text-center'>
                                        <h3 className='text-center'>Sold for <strong>{euroCurrencyValueFormatterToNumber(item.ad.price)}</strong></h3>
                                    </Card.Body>
                                    <Card.Footer style={{width: 300}}>
                ...
                <div className="action-wrapper text-center mt-4">
                    {
                        statsData.stats &&
                        <Button onClick={() => generatePDF(targetRef, {filename: 'report.pdf'})} className='btn btn-success'>
                            Export to PDF
                        </Button>
                    }
                </div>
